...
private:
	...
	std::array<unsigned int, 64> nesRGB;
public:
	ExampleLayer()
		: Layer("GameLayer")
	{
		Shader shader("res/shaders/" Shader("2D"));
		renderer = new BatchRenderer(256, 240, shader);
	...
	void OnUpdate() override
	{
		...
		for (int i = 0; i < nesRes.x; i += 8) {
			for (int j = 0; j < nesRes.y; j += 8) {
				renderer->DrawRectangle(glm::vec2(8, 8), glm::vec2(i, j), HexToRgb(nesRGB[rand() % 64]));
			}
		}
